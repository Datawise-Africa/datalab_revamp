...
const DownloadDataModal = ({ dataset }) => {
  ...
  const handleInputChange = (event ) => {
    ...
    if (name === 'email_address') { // Check if it's the email input
      setEmail(value); 
    ...
    const handleAction = async () => {
      ...
      try {
        ...
        if (!codeSent) {
          ...
          try {
            const response = await fetch(`${REACT_PUBLIC_API_HOST}/data/generate-otp/`, {
              method: "POST",
              headers: {
                "Accept": "application/json",
                "Content-Type": "application/json",
          ...
          try {
            const response = await fetch(`${REACT_PUBLIC_API_HOST}/data/verify-otp/`, {
              method: "POST",
              headers: {
                "Accept": "application/json",
                "Content-Type": "application/json",
            ...
            if (response.ok && data.message.toLowerCase().includes("otp validated successfully")) {
    ...
    const handleStarClick = async (value) => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          setMessage(`Failed to submit rating: ${data.detail || "Unknown error"}`);
        }
      } catch (error) {
        ...
        <div key={index} className="flex items-center">
          <div
            className={`flex flex-col items-center ${
              index + 1 === currentStep ? "text-[#ddeeff]" : "text-[#ddeeff]"
            }`}
          >
            <div
              className={`flex items-center justify-center w-8 h-8 rounded-full border-2 ${
                index + 1 === currentStep
                  ? "bg-n-15 text-[#ddeeff] border-[#004176]"
                  : "bg-n-6 text-[#ddeeff] border-[#0E0C15]"
              }`}
            >
              {index + 1}
            </div>
